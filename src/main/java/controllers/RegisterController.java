...
public class RegisterController {
    ...
    public String register_farmer() {
        try {
            ...
            Admin a = AdminModel.find_admin_by_username(username);

            boolean username_not_taken = f == null && c == null && a == null;

            if (verify && password_pass && firstName && lastName && email_check && phone_num && username_not_taken) {
                ...
                //RequestModel.add_request(username, 1);
                return "success";
            } else {
                ...
                if (!verify) {
                    FacesContext.getCurrentInstance().addMessage(null,
                            new FacesMessage(FacesMessage.SEVERITY_ERROR, "You must verify you are not robot.", "You must verify you are not robot."));
                }
    ...
    public String register_company() {
        try {
            ...
            //System.out.println("number: " + contact_number);
            Farmer f = FarmerModel.find_farmer_by_username(short_name);
            Company c = CompanyModel.find_company_by_short_name(short_name);
            Admin a = AdminModel.find_admin_by_username(short_name);
            
            boolean username_not_taken = f == null && c == null && a == null;

            if (verify && password_pass && email_check && username_not_taken) {
